...
namespace VladislavTsurikov.AddressableLoaderSystem.Tests
{
    ...
    public class SceneAConfigLoader : BindableResourceLoader
    {
        ...
        public override async UniTask LoadResourceLoader(CancellationToken token)
        {
            ...
            ConfigWithReference = await LoadAndBind<ConfigSceneAWithAssetReference>(token,"ConfigSceneA_WithAssetReference");
            DictionarySpriteConfig = await LoadAndBind<DictionarySpriteConfigSceneA>(token,"DictionarySpriteConfigSceneA");
        ...
        {
            if (DictionarySpriteConfig == null || DictionarySpriteConfig.Sprites == null || !DictionarySpriteConfig.Sprites.Any())
            {
                Debug.LogError("[SceneAConfigLoader] DictionarySpriteConfig or its ChapterImages dictionary is null or empty!");
                return;
            }

            foreach (var entry in DictionarySpriteConfig.Sprites)
            {
                ...
                {
                    Debug.Log($"[SceneAConfigLoader] Sprite for key '{entry.Key}' loaded successfully. Type: {entry.Value.Asset.GetType()}");
                }
            }
        }
    }
